...
void CompositionalMultiphaseHybridFVM::InitializeFluidState( MeshLevel & mesh ) const
{
  CompositionalMultiphaseBase::InitializeFluidState( mesh );
  /*
     ...
     forAll< serialPolicy >( facePres.size(), [=] GEOSX_HOST_DEVICE ( localIndex const iface )
     {
     ...
     if( !onBoundary )
        {
     facePres[iface] += m_pressure[er][esr][ei];
          elemCounter++;
        }
      }
      if( elemCounter >= 1 )
      {
        facePres[iface] /= elemCounter;
     if( iface == 126417 || iface == 135260 || iface == 136980 )
        {
          if( elemCounter == 1 )
     {
      std::cout << "face #" << iface << " is on the boundary" << std::endl;
     }
     else
          {
      std::cout << "face #" << iface << " is not on the boundary" << std::endl;
     }
     }
      }
     }
                                                                   ...
                                                                   arrayView1d< real64 const > const & localSolution )
{
  ...
  std::cout << "res scalor facting = " << scalingFactor << std::endl;
