...
void CornerPointMeshPartition::scatterPartitionBoundaries( CornerPointMeshDimensions & dims )
{
  ...
  if( m_myRank == m_mainRank )
  {
    ...
    for( localIndex iy = 0; iy < m_size; ++iy )
    {
      localIndex const rank = iy;
      boundaries( rank*sizePerRank )   = 0;
      boundaries( rank*sizePerRank+1 ) = iy * ySpacing;
      boundaries( rank*sizePerRank+2 ) = dims.nX()-1;
      boundaries( rank*sizePerRank+3 ) = ( iy < m_size-1 ) ? ( iy+1 ) * ySpacing - 1 : dims.nY()-1;
      
      overlaps( rank*sizePerRank ) = 0;
      overlaps( rank*sizePerRank+1 ) = ( boundaries( rank*sizePerRank ) == 0 ) ? 0 : 1;
      overlaps( rank*sizePerRank+2 ) = 0;
      overlaps( rank*sizePerRank+3 ) = ( boundaries( rank*sizePerRank+2 ) == dims.nY()-1 ) ? 0 : 1;

      neighbors( rank*sizePerRank )   = -1;
      neighbors( rank*sizePerRank+1 ) = ( iy > 0 ) ? iy-1 : -1;
      neighbors( rank*sizePerRank+2 ) = -1;
      neighbors( rank*sizePerRank+3 ) = ( iy < m_size-1 ) ? iy+1 : -1;
    }
